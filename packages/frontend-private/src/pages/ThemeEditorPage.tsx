...
import { useTheme, useAuth, themes, Button, ColorPicker } from '@atelier/shared-ui'
...
const VAR_LABELS: Array<{ key: string; label: string }> = [
  ...
  { key: '--color-on-primary', label: 'Sur primaire' },
]
...
export function ThemeEditorPage() {
  ...
  const [draft, setDraft] = useState<Record<string, string>>(() => ({ ...themes['light'], ...customColors }))

  const handleChange = (key: string, value: string) => setDraft(prev => ({ ...prev, [key]: value }))

  const handleRandomAll = () => setDraft(Object.fromEntries(VAR_LABELS.map(({ key }) => [key, randomHex()])))
        ...
        <section style={panelStyle}>
          {VAR_LABELS.map(({ key, label }) => (
            <ColorPicker
              key={key}
              label={label}
              // Repli littéral : <input type="color"> n'accepte pas une var CSS.
